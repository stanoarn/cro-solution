...
3. Upgraduj a instaluj knihovny pro běh skriptu
...
4. Spusť skript a vygeneruj pracovní data

   ```powershell
   python prepare.py
   ```

Skript vytvoří v adresáři, ze kterého je spouštěn, nový adresář  `source` [^2] s testovacími soubory ve formátu JSON. Očekávejte více než jeden dva tisíce souborů, kdy pro každý den od 1. ledna 2021 do aktuálního data vytvoří jeden až pět souborů.
...
Vaším prvním úkolem je přemístit soubory ze zdrojového adresáře `source` do cílového adresáře `target`, ve kterém je uspořádáte podle roku a čísla týdne za pomoci JSON atributu `date`. Dále přejmenujte souboru tak, že odstraníte rok z názvu souboru např. z `2021-12-31_1.json` bude `12-31_1.json`.
...
- Program by měl na příkazové řádce přebírat následující dva argumenty:
  ...
  - `--output (-o)`: cesta k adresáři do kterého chceme ukládat soubory, např. `target`.
     ...
     **Příklad**
      ...
      Pokud není argument zadán, zkontrolujte, jestli není nastavena proměnná prostředí (*environment variable*) s názvem `SOURCE_DIRECTORY` respektive `TARGET_DIRECTORY` a případně použijte jejich hodnotu. Jinak nastavte výchozí hodnotu na aktuální adresář, ze kterého je program spouštěn.
...
- Program by měl případou chybu při přemístění souboru vypisovat (logovat) do konzole (`stderr`), ale měl by pokračovat dále.

- Program by měl po skončení vypsat (`stderr`) kolik celkem souborů přemístil.
  Pokud nezpracoval všechny soubory měl by vracet status kód různý od nuly.
...
- Program by měl ve výchozím stavu jen vypisovat (*stdout*), které změny udělá tzn. implementovat [*dry run*](https://en.wikipedia.org/wiki/Dry_run_(testing)). Pokud chceme změny opravdu provést, použijeme přepínač `--write (-w)`
...
Pokud správně používáte `stderr` a `stdout`, mělo by být možně přesměřovat výstup programu do vstupu dalšího programu nebo souboru např.
